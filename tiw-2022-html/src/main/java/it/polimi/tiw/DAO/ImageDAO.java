...
public class ImageDAO {
	...
	public List<Image> getAlbumImages(String albumOwner, String albumTitle, int pageOffset) throws SQLException {
		String query = "SELECT * FROM immagine WHERE proprietario_album = ? AND titolo_album = ? ORDER BY data DESC LIMIT 5 OFFSET " + pageOffset;
		try (PreparedStatement pstatement = connection.prepareStatement(query);) {
	...
	/**
	 * Controlla se esiste almeno un'immagine successiva a quelle correntemente
	 * visualizzate.
	 * 
	 * @param albumOwner
	 * @param albumTitle
	...
	public boolean hasNextSetOfFive(String albumOwner, String albumTitle, int pageOffset) throws SQLException{
		String query = "SELECT * FROM immagine WHERE proprietario_album = ? AND titolo_album = ? ORDER BY data DESC LIMIT 5 OFFSET " + pageOffset;
		try (PreparedStatement pstatement = connection.prepareStatement(query);) {
			pstatement.setString(1, albumOwner);
			pstatement.setString(2, albumTitle);
