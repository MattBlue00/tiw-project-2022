...
public class UserDAO {
	...
	public User checkCredentials(String username, String password) throws SQLException {
		String query = "SELECT * FROM database.utente WHERE email = ? AND password =?";
	...
	public boolean checkCredentialsRegistration(String email, String username) throws SQLException {
		String query = "SELECT * FROM database.utente WHERE username = ? OR email =?";
		try (PreparedStatement pstatement = connection.prepareStatement(query);) {
			pstatement.setString(1, username);
	...
	public void registerUser(String username, String email, String password) throws SQLException {
		String query = "INSERT INTO database.utente (username, email, password) VALUES(?, ?, ?)";
		try (PreparedStatement pstatement = connection.prepareStatement(query);) {
			pstatement.setString(1, username);
			pstatement.setString(2, email);
